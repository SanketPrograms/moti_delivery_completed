...
class TotalPendingOrder extends StatefulWidget {
  TotalPendingOrder({Key? key}) : super(key: key);
...
class _TotalPendingOrderState extends State<TotalPendingOrder> {
      ...
      loaderWidget(),) :orderHistoryModalclass.result!.length==0?Card(
        child: Column(
          ...
          children: [
            ...
            Text("No Pending Orders Found!",style: GoogleFonts.poppins(
  ...
  getData() async{
    ...
    var response = await http.get(Uri.parse("$viewOrderApi?userid=$userID&mode=received"));
